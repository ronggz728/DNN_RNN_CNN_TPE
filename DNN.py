...
def main():
    ...
    model.add(keras.layers.Dense(units=35, input_dim=17, kernel_initializer=init, activation='relu'))
    model.add(keras.layers.Dense(units=35, kernel_initializer=init, activation='relu'))
    model.add(Dropout(0))
    ...
    model.fit(X_train, Y_train, shuffle=True, validation_data=(X_test, Y_test), verbose=1, callbacks=[history],batch_size=64,epochs=50)
    ...
    exp_1.to_csv("F://DNN//ALL_1.csv")

    exp_2 = DataFrame(pre_test_data_2)
    exp_2.to_csv("F://DNN//ALL_2.csv")

    exp_3 = DataFrame(pre_test_data_3)
    exp_3.to_csv("F://DNN//ALL_3.csv")

    exp_4 = DataFrame(pre_test_data_4)
    exp_4.to_csv("F://DNN//ALL_4.csv")

    exp_5 = DataFrame(pre_test_data_5)
    exp_5.to_csv("F://DNN//ALL_5.csv")
